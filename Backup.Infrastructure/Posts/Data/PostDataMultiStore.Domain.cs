...
public partial class PostDataMultiStore : IPostDomainData
{
    ...
    {
        List<Backup.Infrastructure.Models.Posts.Post>? posts = await GetAll();
        return posts?.Select(PostReplicationMapper.ToDomain).ToList();
    }

    async Task<List<MediaInput>?> IPostDomainData.GetMediaInputs()
    {
        List<Backup.Infrastructure.Models.Posts.MediaInput>? inputs = await GetMediaInputs();
    ...
    {
        List<Backup.Infrastructure.Models.Posts.Post> posts = await GetByIds(ids);
        return posts.Select(PostReplicationMapper.ToDomain).ToList();
    }
